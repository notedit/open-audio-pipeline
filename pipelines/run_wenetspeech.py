#!/usr/bin/env python
"""WenetSpeech4TTS 清洗入口.

用法:
  python pipelines/run_wenetspeech.py \
      --input "/path/to/WenetSpeech4TTS/Basic/*.tar.gz" \
      --output /path/to/output \
      --config configs/wenetspeech.yaml \
      --device cuda:0

多卡并行方式同 run_emilia.py.
"""
...
from audio_pipeline.datasets import iter_wenetspeech4tts_tar
from audio_pipeline.runner import run_shards, setup_logging


def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--input", required=True, help="输入 tar.gz 分片 glob")
    ap.add_argument("--output", required=True, help="输出目录")
    ap.add_argument("--config", default="configs/wenetspeech.yaml")
    ...
    run_shards(
        shard_paths=shards,
        iter_shard=iter_wenetspeech4tts_tar,
